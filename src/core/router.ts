...
const EMPTY_FROZEN_ARRAY = Object.freeze([]) as readonly string[];
...
export class UpdateRouter {
  ...
  private parseCommand(
    ...
    const args =
      tail === "" ? EMPTY_FROZEN_ARRAY : (Object.freeze(tail.split(/\s+/)) as readonly string[]);
  ...
  private async dispatchIndexedHandlers(
    ...
    if (meta.kind === "message" && update.message) {
      ...
      // optimization: find matching keys using a for...in loop
    ...
    // optimization: process non-message update fields using for...in
  ...
  private getUpdateMetadata(update: Update): { kind: string; chatId?: number } {
    // optimization: direct property checks are faster than for...in for known update fields
