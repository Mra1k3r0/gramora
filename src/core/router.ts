...
export class UpdateRouter {
  ...
  private createContext(
    ...
    switch (handler.kind) {
      ...
      case "callback_query":
        return new CallbackContext(options);
      case "inline_query":
        return new InlineContext(options);
      case "chat_member":
      case "my_chat_member":
  ...
  private parseCommand(
    ...
    // fast-path: avoid trimStart() if it already starts with /
    const trimmed = text[0] === "/" ? text : text.trimStart();
  ...
  private async dispatchIndexedHandlers(
    ...
    // Forward compatibility: find matching keys in O(K) using for...in
    let matchedUpdateKinds: string[] | undefined;
    for (const key in update) {
      if (
        ...
        else matchedUpdateKinds.push(key);
      }
    }

    if (matchedUpdateKinds) {
      if (matchedUpdateKinds.length > 1) {
        matchedUpdateKinds.sort(
          (a, b) =>
            (this.kindRegistrationOrder.get(a) ?? 0) - (this.kindRegistrationOrder.get(b) ?? 0),
        );
      }
      for (const kind of matchedUpdateKinds) {
        const runners = this.onKindHandlers.get(kind);
        if (runners) {
          for (const runner of runners) {
            await this.runControllerRunner(
  ...
  private getUpdateMetadata(update: Update): { kind: string; chatId?: number } {
    // fast-path for the most common update types using an if-chain
    // this avoids the overhead of property enumeration in the for...in loop
    if (update.message) {
      return { kind: "message", chatId: update.message.chat.id };
    }
    ...
    if (update.edited_message) {
      return { kind: "edited_message" };
    }
    if (update.inline_query) {
      return { kind: "inline_query" };
    }
    if (update.chat_member) {
      return { kind: "chat_member", chatId: update.chat_member.chat.id };
    }
    ...
    // fallback for rare or new update types
    for (const key in update) {
      if (
        key === "update_id" ||
        !Object.prototype.hasOwnProperty.call(update, key) ||
